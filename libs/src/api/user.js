...
const {
  getSpIDForEndpoint,
  setSpIDForEndpoint,
} = require("../services/creatorNode/CreatorNodeSelection");
...
class Users extends Base {
  ...
  async getUsers(
    ...
    return this.discoveryProvider.getUsers(
      limit,
      offset,
      idsArray,
      walletAddress,
  ...
  async getMutualFollowers(limit = 100, offset = 0, followeeUserId) {
    ...
    if (followerUserId) {
      return this.discoveryProvider.getFollowIntersectionUsers(
        limit,
        offset,
        followeeUserId,
        followerUserId
  ...
  async getFollowersForUser(limit = 100, offset = 0, followeeUserId) {
    this.REQUIRES(Services.DISCOVERY_PROVIDER);
    return this.discoveryProvider.getFollowersForUser(
      limit,
      offset,
      followeeUserId
    );
  ...
  async getFolloweesForUser(limit = 100, offset = 0, followerUserId) {
    this.REQUIRES(Services.DISCOVERY_PROVIDER);
    return this.discoveryProvider.getFolloweesForUser(
      limit,
      offset,
      followerUserId
    );
  ...
  async getUserRepostFeed(
    ...
    this.REQUIRES(Services.DISCOVERY_PROVIDER);
    return this.discoveryProvider.getUserRepostFeed(
      userId,
      filter,
      limit,
      offset,
  ...
  async getSocialFeed(
    ...
    if (owner) {
      return this.discoveryProvider.getSocialFeed(
        filter,
        limit,
        offset,
        withUsers,
  ...
  async getTopCreatorsByGenres(
    ...
    this.REQUIRES(Services.DISCOVERY_PROVIDER);
    return this.discoveryProvider.getTopCreatorsByGenres(
      genres,
      limit,
      offset,
      withUsers
  ...
  async assignReplicaSet({ userId }) {
    ...
    const phases = {
      ...
      UPLOAD_METADATA_AND_UPDATE_ON_CHAIN:
        "UPLOAD_METADATA_AND_UPDATE_ON_CHAIN",
    };
    let phase = "";
  ...
  async updateUser(userId, metadata) {
    ...
    let users = await this.discoveryProvider.getUsers(
      1,
      0,
      [userId],
      null,
  ...
  async updateCreator(userId, metadata) {
    ...
    if (
      ...
      const updateEndpointTxReceipt = await this._updateReplicaSetOnChain(
        userId,
        newMetadata.creator_node_endpoint
      );
  ...
  async updateIsVerified(userId, isVerified, privateKey) {
    return this.contracts.UserFactoryClient.updateIsVerified(
      userId,
      isVerified,
      privateKey
    );
  ...
  async addUserFollow(followeeUserId) {
    const followerUserId = this.userStateManager.getCurrentUserId();
    return this.contracts.SocialFeatureFactoryClient.addUserFollow(
      followerUserId,
      followeeUserId
    );
  }
  ...
  async deleteUserFollow(followeeUserId) {
    const followerUserId = this.userStateManager.getCurrentUserId();
    return this.contracts.SocialFeatureFactoryClient.deleteUserFollow(
      followerUserId,
      followeeUserId
    );
  }
  ...
  async updateAndUploadMetadata({ newMetadata, userId }) {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA update replica s3t"
        );
        ...
        console.log(
          "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA WAITING FOR CN ENDPOINT INDEXING"
        );
        // Ensure DN has indexed creator_node_endpoint change
        await this._waitForCreatorNodeEndpointIndexing(
          userId,
      ...
      console.log(
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA UPLOAD METADATAAAA"
      );
      // Upload new metadata object to CN
      ...
      throw new Error(
        `updateAndUploadMetadata() Error -- Phase ${phase}: ${e}`
      );
    }
  }
  ...
  async _waitForCreatorNodeEndpointIndexing(userId, creatorNodeEndpoint) {
    ...
    while (!isUpdated) {
      ...
      console.log(`${userList} ${user} ${userId} ${Date.now()}`);
  ...
  async _addUserOperations(userId, newMetadata, exclude = []) {
    ...
    if (metadata[USER_PROP_NAME_CONSTANTS.NAME]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateName(
          userId,
          metadata[USER_PROP_NAME_CONSTANTS.NAME]
        )
      );
    }
    if (metadata[USER_PROP_NAME_CONSTANTS.LOCATION]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateLocation(
          userId,
          metadata[USER_PROP_NAME_CONSTANTS.LOCATION]
        )
      );
    }
    if (metadata[USER_PROP_NAME_CONSTANTS.BIO]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateBio(
          userId,
          metadata[USER_PROP_NAME_CONSTANTS.BIO]
        )
      );
    }
    if (metadata[USER_PROP_NAME_CONSTANTS.PROFILE_PICTURE_SIZES]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateProfilePhoto(
          userId,
          Utils.decodeMultihash(
            metadata[USER_PROP_NAME_CONSTANTS.PROFILE_PICTURE_SIZES]
          ).digest
        )
      );
    }
    if (metadata[USER_PROP_NAME_CONSTANTS.COVER_PHOTO_SIZES]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateCoverPhoto(
          userId,
          Utils.decodeMultihash(
            metadata[USER_PROP_NAME_CONSTANTS.COVER_PHOTO_SIZES]
          ).digest
        )
      );
    }
    if (metadata[USER_PROP_NAME_CONSTANTS.IS_CREATOR]) {
      addOps.push(
        this.contracts.UserFactoryClient.updateIsCreator(
          userId,
          metadata[USER_PROP_NAME_CONSTANTS.IS_CREATOR]
        )
      );
    }
  ...
  async _updateUserOperations(
    ...
    for (const key in metadata) {
      if (
        ...
        currentMetadata.hasOwnProperty(key) &&
        metadata[key] !== currentMetadata[key]
      ) {
  ...
  async _updateReplicaSetOnChain(userId, creatorNodeEndpoint) {
    ...
    if (!this.contracts.UserReplicaSetManagerClient) {
      const { txReceipt: updateEndpointTxReceipt } =
